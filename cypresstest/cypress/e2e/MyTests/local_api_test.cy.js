...
describe('Check Local API', () => {
    ...
    // it('Visit the URL', () => {
    //     cy.visit(url);
    //     cy.get('#LocalTable').should('length', 1);
    // })
    ...
    //     cy.get('#LocalTable > tbody > tr').eq(0).find('td').first()
    //     .then(($res) => {
    //         rowid =  $res.text();

    //         //Click The Edit Button to Show Modal
    //         cy.get('#LocalTable > tbody > tr').eq(0).find('td').eq(4)
    ...
    it('Check Update Account by Search Button', () => {
        ...
        cy.get('#LocalTable > tbody > tr').last().find('td').first()
        .then((tdText) => {
            //Check if Account exists using a sample ID
    ...
    it('Check Delete Row Button', () => {
        ...
        //Click Delete Button
        cy.get('#LocalTable > tbody > tr').last().find('td').last()
